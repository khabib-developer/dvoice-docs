...
The **Stream TTS** endpoint lets you stream audio as it is generated over a WebSocket connection.
...
- **text**: A string containing the text to convert into speech.
- **model**: The voice model (e.g., `"Iroda"`, `"Surayyo_v2"`).
...
- **data**: Object for model-specific options.
  - For `Surayyo_v2`, you can pass an emotion field `emo` with values:
    - `Happy`
    - `Strict`
    - `Neural`
